locals {
  ...
  security_group_ids = var.create && var.create_security_group ? concat(var.security_group_ids, [aws_security_group.aesthetic-sg[0].id]) : var.security_group_ids
}
...
resource "aws_vpc_endpoint" "aesthetics-endpoint" {
...
resource "aws_security_group" "aesthetic-sg" {
...
resource "aws_security_group_rule" "basic-https-aesthetic" {
  for_each = { for k, v in var.security_group_rules : k => v if var.create && var.create_security_group }

  region = var.region

  security_group_id = aws_security_group.aesthetic-sg[0].id
